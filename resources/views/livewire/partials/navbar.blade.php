    ...
    <div class="relative md:flex md:items-center md:justify-between">
      ...
      <div id="navbar-collapse-with-animation" class="hs-collapse hidden transition-all duration-300 basis-full grow md:block">
        <div class="flex flex-col md:flex-row md:items-center md:justify-end md:gap-x-7 md:ps-7">
          ...
          <!-- Navigation Links -->
          <a wire:navigate class="font-medium {{ request()->is('/') ? 'text-green-600' : 'text-gray-500 hover:text-gray-400' }} py-3 md:py-6" href="/">Home</a>
          
          <a wire:navigate class="font-medium {{ request()->is('categories') ? 'text-green-600' : 'text-gray-500 hover:text-gray-400' }} py-3 md:py-6" href="/categories">Categories</a>
          
          <a wire:navigate class="font-medium {{ request()->is('products*') ? 'text-green-600' : 'text-gray-500 hover:text-gray-400' }} py-3 md:py-6" href="/products">Products</a>
          
          <!-- Cart -->
          <a wire:navigate class="flex items-center font-medium {{ request()->is('cart') ? 'text-green-600' : 'text-gray-500 hover:text-gray-400' }} py-3 md:py-6 transition duration-200" href="/cart">
            🛒 <span class="ml-1">Cart</span> 
            <span class="py-0.5 px-2 rounded-full text-xs font-medium bg-green-100 text-green-700 ml-2">{{$total_count}}</span>
          </a>
          ...
          <div class="relative flex items-center space-x-4">
            ...
            @guest
            <!-- Login Button -->
            <a wire:navigate class="py-2.5 px-4 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-green-600 text-white hover:bg-green-700 disabled:opacity-50 disabled:pointer-events-none" href="/login">
            ...
            <div x-data="{ open: false }" class="relative">
              ...
              <div x-show="open" @click.away="open = false"
                ...
                <a wire:navigate href="{{url('/my-appointments')}}" class="block px-4 py-3 text-sm hover:bg-red-50 hover:text-red-700 transition">📅 Appointments</a>
